...
resolve_source_dir() {
    ...
    local url="https://github.com/GATech-OMSA/Mole/archive/refs/heads/main.tar.gz"

    if [[ "$branch" == "dev" ]]; then
        url="https://github.com/GATech-OMSA/Mole/archive/refs/heads/dev.tar.gz"
    elif [[ "$branch" != "main" ]]; then
        url="https://github.com/GATech-OMSA/Mole/archive/refs/tags/${branch}.tar.gz"
    fi
    ...
    if command -v git > /dev/null 2>&1; then
        ...
        if git clone "${git_args[@]}" https://github.com/GATech-OMSA/Mole.git "$tmp/mole" > /dev/null 2>&1; then
            stop_line_spinner
            SOURCE_DIR="$tmp/mole"
            return 0
        fi
...
get_source_commit_hash() {
    ...
    curl -fsSL --connect-timeout 3 \
        "https://api.github.com/repos/GATech-OMSA/Mole/commits/main" 2> /dev/null |
...
get_latest_release_tag() {
    ...
    tag=$(curl -fsSL --connect-timeout 2 --max-time 3 \
        "https://api.github.com/repos/GATech-OMSA/Mole/releases/latest" 2> /dev/null |
...
get_latest_release_tag_from_git() {
    ...
    git ls-remote --tags --refs https://github.com/GATech-OMSA/Mole.git 2> /dev/null |
        awk -F/ '{print $NF}' |
        grep -E '^V[0-9]' |
        sort -V |
        tail -n 1
...
download_binary() {
    ...
    local url="https://github.com/GATech-OMSA/Mole/releases/download/V${version}/${binary_name}-darwin-${arch_suffix}"
